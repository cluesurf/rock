...
export function Dock({ name, className, terminalOptions }: DockProps) {
  ...
  useEffect(() => {
    ...
    const id = requestAnimationFrame(() => {
      ...
      if (active && active !== document.body) {
        ...
        const inTree = active.closest('[data-term-tree]')
  ...
  useEffect(() => {
    ...
    // the OS shell.
    term.registerLinkProvider(makeTermLinkProvider(term))
    ...
    // the term:cwd-change IPC so state-store can persist it.
    //
    ...
    term.parser.registerOscHandler(7, payload => {
      ...
      if (cwd && slabId) {
        void termBase()?.notifyCwd?.(slabId, cwd)
      }
...
function termBase(): TermBase | undefined {
  return (window as unknown as { app?: TermBase }).app
}

function makeTermLinkProvider(term: Terminal): ILinkProvider {
  return {
    provideLinks(bufferLineNumber, callback) {
      ...
      for (const m of text.matchAll(URL_RE)) {
        ...
        links.push({
          ...
          activate: () => {
            void termBase()?.openExternal?.(cleanUrl)
          },
        })
      }
      ...
      for (const m of text.matchAll(PATH_RE)) {
        ...
        links.push({
          ...
          decorations: { underline: true, pointerCursor: true },
          activate: () => {
            void termBase()?.openPath?.(filePath)
          },
        })
      }
...
type TermPreloadBridge = {
  getPathForFile?: (file: File) => string
}

function pathForFile(file: File): string | undefined {
  const bridge = (globalThis as unknown as { app?: TermPreloadBridge }).app
