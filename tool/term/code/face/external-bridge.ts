/**
 * Renderer-side: expose React + the term face surface on
 * `globalThis.__term__` so JIT-bundled user code can
...
export function installExternalBridge(): void {
  ...
  const bridge: Record<string, unknown> = {
    ...
    '@cluesurf/term/face': Face,
  }

  ;(globalThis as { __term__?: Record<string, unknown> }).__term__ =
    bridge
}
 ...
 * user `.tool/term/layout.tsx` might want to import). Keys must
 ...
 * the term:// protocol. Returns the DEFAULT export.
 *
...
export async function loadUserModule<T = unknown>(
  ...
  const url = `term://user/${bundleKey}`
  const mod = (await import(/* @vite-ignore */ url)) as {
    default?: T
  }
  if (mod.default === undefined) {
    throw new Error(
      `term://user/${bundleKey} has no default export`,
    )
  }
  return mod.default
}
