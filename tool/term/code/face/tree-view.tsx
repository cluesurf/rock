...
const END_DROP_ID = '__term_tree_end__'
  ...
  return (
    ...
    >
      <div
        ...
        // is no longer inside [data-term-tree], so the
    ...
    >
      ...
      >
        {isRenaming ? (
          ...
          <>
            <span data-term-branch-label="">{node.label}</span>
...
function LeafRow(props: NodeRowProps & { node: LeafNode }) {
  ...
  function handleKeyDown(event: ReactKeyboardEvent<HTMLButtonElement>) {
    if (event.key === 'ArrowDown') {
      ...
      const candidates = document.querySelectorAll<HTMLTextAreaElement>(
        '[data-term-dock][data-ready="true"] .xterm-helper-textarea',
      )
  ...
  return (
    <button
      ...
      onFocus={() => setFocusedId(node.id)}
    >
      <span data-term-leaf-label="">{displayLabel}</span>
...
function DragGhost({ node }: { node: TreeNode }) {
  ...
  return (
    <div
      data-term-tree-ghost=""
      style={{
        display: 'flex',
        alignItems: 'center',
        gap: 6,
        padding: '4px 10px',
        background: 'var(--term-active-bg, #3f3f46)',
        color: 'var(--term-fg, #e4e4e7)',
        borderRadius: 4,
        fontSize: 13,
        fontFamily: 'var(--term-font, ui-monospace, monospace)',
        // Visible ring + drop shadow so the ghost reads as
        // "lifted" against any background.
        boxShadow:
          '0 0 0 1px var(--term-accent, #8b5cf6), 0 6px 16px rgba(0,0,0,0.45)',
