/**
 * JIT bundler for user `.tool/term/layout.tsx` (and other user
 * .tsx files). Runs in the Electron main process.
 *
 * Why bundle: the user writes JSX + TypeScript that
 * imports `@cluesurf/term/face`, `react`, etc. We need to
 ...
 * with reads from `globalThis.__term__`. The renderer
 * populates that object before mounting (see
 * `code/face/external-bridge.ts`). The compiled bundle is
 * served via the `term://` protocol (see
 * `code/desktop/term-protocol.ts`).
 */
 ...
 * exposes on `globalThis.__term__[moduleId]`. ESM requires
...
const SHARED_EXPORTS: Record<string, readonly string[]> = {
  ...
  '@cluesurf/term': ['workspace', 'defineWorkspace'],
  '@cluesurf/term/face': [
    'Slab',
    'Nest',
    'Dock',
  ...
  return {
    ...
    setup(b) {
      ...
      b.onResolve({ filter }, args => ({
        path: args.path,
        namespace: 'term-shared',
      }))
      b.onLoad({ filter: /.*/, namespace: 'term-shared' }, args => {
        ...
        const namedRe = named
          ...
          .join('\n')
        const contents = `
const m = globalThis.__term__ && globalThis.__term__[${safe}]
if (!m) throw new Error('term: shared module not provided by renderer: ' + ${safe})
