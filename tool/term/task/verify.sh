...
BASE="$ROOT/base"
APP="$BASE/dist/mac-arm64/Term.app"
LOG="$(mktemp -t term-verify.XXXXXX.log)"
...
TERM_BIN="$APP/Contents/Resources/term"
[ -x "$TERM_BIN" ] || fail "bundled CLI launcher missing at $TERM_BIN"
...
# <path-to-term.js>" and exits non-zero. We capture both
# stdout and stderr because yargs writes the error to
# stderr.
if ! "$TERM_BIN" --version >"$LOG" 2>&1; then
  fail "term --version exited non-zero"
fi
if grep -qE "Unknown argument|Unknown command" "$LOG"; then
  fail "term --version triggered yargs unknown-argument bug (CLI argv slicing is wrong)"
fi
green "  term --version: OK ($(cat "$LOG" | tr -d '\n'))"

# --help also forces yargs to fully build the command
# tree, catching any subcommand wiring issues.
if ! "$TERM_BIN" --help >"$LOG" 2>&1; then
  fail "term --help exited non-zero"
fi
if grep -qE "Unknown argument|Unknown command" "$LOG"; then
  fail "term --help triggered yargs unknown-argument bug"
fi
green "  term --help: OK"
...
ELECTRON_BIN="$APP/Contents/MacOS/Term"
...
if ! security find-generic-password -s "Term Safe Storage" >/dev/null 2>&1; then
