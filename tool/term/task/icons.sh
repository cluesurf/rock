#!/usr/bin/env bash
# Generate platform app icons from view/term.svg or view/term.png.
#
...
if [ -f "$VIEW/term.svg" ]; then
  SRC_SVG="$VIEW/term.svg"
  echo "source: $SRC_SVG (vector)"
else
  echo "ERROR: no source icon at $VIEW/term.svg" >&2
  exit 1
fi
