#!/usr/bin/env zsh
# Release Term.app to GitHub Releases + Homebrew tap.
# Uses a fine-grained PAT + curl. No gh CLI, no broad
# OAuth scopes.
#
# What it does:
#   1. Reads version from base/package.json.
#   2. Builds Term.app + .dmg + .zip via electron-builder.
#   3. POSTs a Release to cluesurf/term via the GitHub API.
#   4. Uploads .dmg / .zip / .blockmap / latest-mac.yml.
#   5. Clones / pulls the homebrew-tool tap, rewrites
#      Casks/term.rb with new version + sha256, commits,
...
#
# Save the token in `term/.env` (gitignored):
#
...
# ── Env vars ────────────────────────────────────────────
#
#   GITHUB_REPO_TOKEN    required — read from term/.env or shell env
#   TERM_VERSION         override version (default: from package.json)
#   TERM_REPO            release repo (default: cluesurf/term)
#   TERM_TAP_REPO        tap repo (default: cluesurf/homebrew-tool)
#   TERM_TAP_DIR         local tap clone (default: ~/.cache/term-tap)
#   TERM_SKIP_BUILD=1    skip the build step
#   TERM_SKIP_RELEASE=1  skip the GitHub Release step
#   TERM_SKIP_CASK=1     skip the cask update step
#
# Usage:
#   pnpm ship
...
# Load term/.env into the shell environment. The file is
...
REPO="${TERM_REPO:-cluesurf/term}"
TAP_REPO="${TERM_TAP_REPO:-cluesurf/homebrew-tool}"
TAP_DIR="${TERM_TAP_DIR:-$HOME/.cache/term-tap}"
VERSION="${TERM_VERSION:-$(node -p "require('$BASE/package.json').version")}"
...
# ── Build ────────────────────────────────────────────────
if [ "${TERM_SKIP_BUILD:-0}" != "1" ]; then
  step "build Term.app (this is slow — 30s+ for electron-builder)"
  (cd "$ROOT" && pnpm package:mac)
fi

# Resolve built artifact names (electron-builder names them
# differently between versions; probe).
DMG="$(ls "$DIST"/Term-*"$VERSION"*.dmg 2>/dev/null | head -1 || true)"
ZIP="$(ls "$DIST"/Term-*"$VERSION"*mac.zip 2>/dev/null | head -1 || true)"
BLOCKMAP="$(ls "$DIST"/Term-*"$VERSION"*mac.zip.blockmap 2>/dev/null | head -1 || true)"
...
# ── GitHub Release ───────────────────────────────────────
if [ "${TERM_SKIP_RELEASE:-0}" != "1" ]; then
  step "github release"

  # Push the git tag if absent.
  if ! git -C "$ROOT" rev-parse "$TAG" >/dev/null 2>&1; then
    git -C "$ROOT" tag -a "$TAG" -m "Term $VERSION"
...
if [ "${TERM_SKIP_CASK:-0}" != "1" ]; then
  ...
  if git -C "$TAP_DIR" diff --quiet "$CASK"; then
    ...
    git -C "$TAP_DIR" commit -am "term $VERSION"
...
echo
echo "── done ──"
echo "Term $VERSION shipped."
echo
echo "Users install with:"
echo "  brew tap cluesurf/tool"
echo "  brew install --cask cluesurf/tool/term"
echo
echo "Existing users upgrade with:"
echo "  brew upgrade cluesurf/tool/term"
