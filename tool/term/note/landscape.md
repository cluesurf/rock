...
What's out there, what each one optimizes for, and where **Term** fits.
...
Term is squarely in the **Electron + xterm.js + node-pty** quadrant. So
it inherits:
...
- **The entire UI is a React tree you author yourself.** Not a plugin
  API. Not Lua hooks. JSX components from `@cluesurf/term/face` that you
  import and recompose.
- **Native TypeScript ergonomics.** Type your slabs, your layouts, your
  custom commands.
- **JIT-compiled customization** at `.tool/term/code/index.tsx`. Save the
...
ship it as a Mac app, Term is the only one that says yes.
