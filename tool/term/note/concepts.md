...
The mental model behind Term, mapped 1:1 to the code.
...
Slabs are defined in `.tool/term/code/index.tsx`'s
`workspace({ slabs: {...} })`. New ones are spawned at
runtime via `Cmd+T`, `term spawn <name>`, or the
...
The tree is persisted to `.tool/term/base.local.json` (per
machine) — the initial layout can be checked into
`.tool/term/base.json` (committed) and Term merges them on
load.

See `cluesurf/note/library/term/sidebar-tree-design.md`
...
In the default Term shell, every slab in the tree gets a
...
`$TMPDIR/term.sock` — lets the `term` CLI talk to a
running Term.app. JSON-lines over the socket. Commands:
...
and `base/call/term.ts` (client). The Homebrew cask
installs the CLI; under the hood it's a bash launcher
that execs Term.app's bundled Electron in Node mode
(`ELECTRON_RUN_AS_NODE=1`).

## Workspace persistence

Per-project state lives in `.tool/term/`:
...
A **`TermTheme`** is a plain data object — colors, font,
...
(`--term-bg`, `--term-fg`, `--term-accent`, etc.) so the
...
launch
  ...
  base/boot/index.ts (renderer + main start)
    findToolFolders(cwd)
    loadToolFolder(.rock)   ← workspace
    bundleUserModule(.tool/term/code/index.tsx)
    ...
    boot({ workspace, userBundles })
                            ...
                            code/boot/index.ts (lib)
                              ...
                              startTermIpcServer (for CLI)
                              registerTermProtocol (for term://)
                              createWindow
                                ↓
                                Renderer:
                                  mount <App />
...
| .tool/term/     | The per-project config + state directory                |
| term://    | Custom Electron protocol serving JIT bundles            |
| OSC 7      | Shell escape for cwd reporting; Term uses for tracking  |
