...
import { dirname as dirname3, join as join2, resolve as resolve5, isAbsolute } from "node:path";
...
function callRock(cmd, args) {
  return new Promise((resolveP, reject) => {
    ...
    conn.on("data", (chunk) => {
      ...
      if (idx < 0) return;
      try {
        const res = JSON.parse(buf.slice(0, idx).trim());
        settled = true;
        conn.end();
        resolveP(res);
      } catch (err) {
...
async function cmdOpen(targetPath) {
  if (!existsSync(targetPath)) {
    throw new Error(`not a directory: ${targetPath}`);
  }
  const abs = isAbsolute(targetPath) ? targetPath : resolve5(targetPath);
  ...
  if (sockExists) {
    try {
      ...
      process.stdout.write(
        `${res.ok ? "\u2713" : "\u2715"} Rock.app reachable    ${res.ok ? "pong" : res.error}
`
      );
    } catch (err) {
...
async function cmdInstallTheme(target) {
  const here = dirname3(new URL(import.meta.url).pathname);
  const sourceDir = join2(here, "themes", target);
  ...
  const installs = {
    claude: join2(homedir(), ".claude", "themes")
  };
  ...
  const files = readdirSync3(sourceDir).filter((f) => f.endsWith(".json"));
  ...
  for (const file of files) {
    ...
    writeFileSync(dest, readFileSync4(src, "utf-8"), "utf-8");
...
async function main() {
  ...
  await yargs_default(argv).scriptName("rock").version(VERSION).usage("$0 <cmd> [args]").strict().demandCommand(1).recommendCommands().help().wrap(Math.min(100, process.stdout.columns ?? 100)).command(
    "open [path]",
    ...
    async (args) => {
      await cmdOpen(args.path ?? process.cwd());
    }
  ).command(
    "bind",
    ...
    (y) => y.positional("slab", { type: "string", demandOption: true }).positional("text", { type: "string", array: true, demandOption: true }),
    async (args) => {
      const text = args.text.join(" ");
      await cmdSend(args.slab, text);
    }
  ).command(
    "focus <slab>",
    "Activate a slab",
    ...
    async (args) => {
      if (args.kind === "theme") {
        await cmdInstallTheme(args.target);
      }
    }
  ).command(
    "doctor",
