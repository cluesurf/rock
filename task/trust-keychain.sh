#!/usr/bin/env bash
# trust-keychain.sh — pre-create Rock's "Safe Storage"
...
#
#   When Rock.app is properly Developer-ID-signed +
...
# To undo:
#
#     security delete-generic-password -s "Rock Safe Storage"

set -euo pipefail

ITEM="Rock Safe Storage"
...
security add-generic-password \
  ...
  -j "Rock dev-build placeholder"

echo "Created keychain item '$ITEM' with permissive ACL."
echo
echo "Note: as of the latest Rock build, this script is no longer"
echo "strictly necessary — Rock's main process now disables cookie"
