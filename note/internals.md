...
Rock stands on the shoulders of xterm.js + node-pty instead of rolling
its own.
...
```
┌────────────────────────────────────────────┐
│  3. Workspace UI (sidebar, tabs, layout)   │  ← Rock's focus
├────────────────────────────────────────────┤
...
interesting design lives. Rock skips the bottom two and builds only the
top.
...
**Rock builds layer 3 and uses other people's layers 1 and 2.**
...
| 3 (workspace UI) | **Rock itself** | React tree, sidebar, command palette, hotkeys, .rock/code/ JIT.                                 |
...
(Rock added drag-and-drop tree tabs in an afternoon; doing that in
Alacritty would be months).

If at some point Rock wants the native-speed path, the move is to
