...
Every keyboard shortcut Rock ships with. Modifier
...
## Reserved by macOS (not Rock)

These work because they're system-level — Rock doesn't
override them:

| Keys           | Action                                              |
| -------------- | --------------------------------------------------- |
| `Cmd+Q`        | Quit Rock entirely                                  |
| `Cmd+H`        | Hide Rock                                           |
| `Cmd+M`        | Minimize the window                                 |
| `Cmd+Tab`      | Switch to next app                                  |
| `Cmd+Space`    | Spotlight (or Alfred / Raycast)                     |
| `Cmd+,`        | Preferences (no-op in Rock right now)               |

## Inside the terminal

Standard xterm/zsh behavior — Rock doesn't touch these:
