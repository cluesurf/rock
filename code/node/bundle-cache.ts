/**
 * Bundle cache for `.rock/code/index.tsx`.
 *
 ...
 *
 * Cache files live in `.rock/.cache/`:
 *
 ...
 *
 * `.rock/.cache/` is gitignored automatically by
...
export function readBundleCache(
  ...
  const cacheDir = join(rockFolderPath, CACHE_DIR)
  ...
  try {
    ...
    for (const input of inputs.inputs) {
      const fullPath = resolve(rockFolderPath, '..', input.path)
...
export function writeBundleCache(
  ...
  try {
    ...
    const inputs = result.watchFiles.map(p => {
      const fullPath = resolve(rockFolderPath, '..', p)
    ...
    console.warn(
      '[rock] bundle cache write failed:',
...
export async function bundleUserModuleCached(
  ...
  const cached = readBundleCache(rockFolderPath, entryFile)
  if (cached) return cached
  const result = await bundleUserModule(entryFile)
  if (!result) return null
  writeBundleCache(rockFolderPath, result)
  return result.code
}
