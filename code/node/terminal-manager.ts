...
/**
 * One-shot setup of a Rock-controlled ZDOTDIR. zsh
 * sources $ZDOTDIR/.zshrc instead of ~/.zshrc when this
 * env var is set, so we can inject the OSC 7 cwd hook
 * during shell startup — invisibly. Earlier versions
 * typed the init line into the live PTY post-spawn, which
 * the shell echoed back to the screen (visible garbage).
 *
 * The wrapper rc sources the user's normal config first
 * so their aliases, prompt, completions, etc. all still
 * load.
 */
let cachedZdotdir: string | null = null
function ensureRockZdotdir(): string {
  if (cachedZdotdir) return cachedZdotdir
  const dir = path.join(tmpdir(), `rock-shell-init-${process.pid}`)
  mkdirSync(dir, { recursive: true })
  // .zshrc — sourced for interactive zsh.
  // .zshenv — sourced for ALL zsh invocations (including
  // non-interactive). We only need .zshrc, but writing
  // .zshenv too means the user's HISTFILE override (if
  // any) still resolves correctly because ZDOTDIR is
  ...
  async createSlab(input: {
    ...
    const proc = pty.spawn(program, input.args ?? [], {
      ...
      env: {
        ...
        // forced newline.
        PROMPT_EOL_MARK: '',
        // Point zsh at a Rock-managed rc directory. The
        // rc there sources the user's normal ~/.zshrc
        // then installs the OSC 7 cwd hook. Doing this
        // via ZDOTDIR means the setup happens DURING
        // shell startup — never appears in the terminal.
        // For bash / fish / other shells this env var
        // is harmless (they ignore it); they fall back
        // to their normal rc files and don't get the
        // cwd-tracking hook (initial cwd still works).
        ZDOTDIR: ensureRockZdotdir(),
        ...input.env,
