...
function renderHighlightedText(
  ...
  for (let i = 0; i < text.length; i++) {
    ...
    if (isMatch === bufferMatched) {
      ...
      if (buffer) {
        out.push(
          bufferMatched
            ? <span key={`m${i}`} data-rock-palette-match="">{buffer}</span>
            : buffer
        )
      }
      buffer = ch
  ...
  if (buffer) {
    out.push(
      bufferMatched
        ? <span key="m-tail" data-rock-palette-match="">{buffer}</span>
        : buffer
    )
  }
  return <>{out}</>
          ...
          {results.map((r, i) => {
            ...
            return (
              ...
              >
                ...
                <span data-rock-palette-item-label="">
                  {renderHighlightedText(cmd.label, highlightLabel)}
                </span>
                {hasPath && parents.length > 0 && (
                  <span data-rock-palette-item-path="">
                    {parents.map((seg, segIdx) => (
                      <span key={segIdx} data-rock-palette-path-seg="">
                        {segIdx > 0 && (
                          <span data-rock-palette-path-sep="">{' › '}</span>
                        )}
